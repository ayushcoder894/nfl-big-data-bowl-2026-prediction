# NFL Big Data Bowl 2026 - CPU-Optimized High-Performance Submission Code
...
print(f"Optimizing for {n_cores} CPU cores")
...
# ===================================
# 4. TRAIN CPU-OPTIMIZED MODELS FOR SUB-0.1 RMSE
# ===================================
...
print("Training CPU-optimized high-performance models...")

# Model 1: Optimized XGBoost with all CPU cores
print("Training XGBoost with maximum CPU utilization...")
...
print("Training LightGBM with CPU optimization...")
...
# Model 3: CPU CatBoost
print("Training CatBoost with CPU optimization...")
...
# Model 4: Optimized Neural Network
...
# Model 5: Extra Trees with maximum parallelization
print("Training Extra Trees with full CPU utilization...")
...
# Model 6: Random Forest with full parallelization
print("Training Random Forest with full CPU utilization...")
...
en_model = MultiOutputRegressor(ElasticNet(
    alpha=0.001,
    l1_ratio=0.5,
    max_iter=2000,
    random_state=42
...
def cpu_ensemble_predict(X_test):
    predictions = []
    weights = []
    for name, model in models.items():
        try:
...
if base_predictions:
    ...
    print(f"Stacked ensemble RMSE: {stacked_rmse:.6f} yards")
    
    cpu_ensemble_predict = stacked_predict
else:
    print("Using simple weighted ensemble")

# Final ensemble evaluation
ensemble_pred = cpu_ensemble_predict(X_val)
ensemble_mse = mean_squared_error(y_val, ensemble_pred)
ensemble_rmse = np.sqrt(ensemble_mse)

print(f"\nFinal CPU-Optimized Ensemble Results:")
...
def generate_predictions_cpu_ensemble(test_input_df, test_targets_df, ensemble_predict_func, ensemble_info):
    """Generate predictions using CPU-optimized ensemble model"""
    ...
    for (game_id, play_id), play_targets in test_targets_df.groupby(['game_id', 'play_id']):
        ...
        for _, target_row in play_targets.iterrows():
            ...
            if len(player_input) == 0:
                ...
                # Predict using CPU-optimized ensemble
...
# Generate predictions using CPU-optimized ensemble
print("Generating predictions with CPU-optimized ensemble model...")
submission = generate_predictions_cpu_ensemble(test_input_features, test_targets, cpu_ensemble_predict, ensemble_info)
...
submission.to_csv('submission_cpu_optimized.csv', index=False)

print(f"\nCPU-Optimized High-Performance Submission Summary:")
...
print("\n" + "="*70)
print("🚀 CPU-OPTIMIZED HIGH-PERFORMANCE SUBMISSION CREATED SUCCESSFULLY! 🚀")
print("="*70)
print("File saved as: submission_cpu_optimized.csv")
print(f"Optimized for maximum performance using all {n_cores} CPU cores!")
print("Advanced stacking ensemble with 10 state-of-the-art models!")
print("Ready to achieve sub-0.1 RMSE performance!")
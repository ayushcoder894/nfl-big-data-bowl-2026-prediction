...
generated_bytes, generated_lines = analyze_file("submission (6).csv")
...
if sample_lines != generated_lines:
    ...
    for i, (line1, line2) in enumerate(zip(sample_lines, generated_lines)):
        if line1 != line2:
            ...
            print(f"      submission (6).csv:    {line2[:100]}")
...
print(f"   submission (6).csv uses:    {'CRLF (Windows)' if generated_has_crlf else 'LF (Unix)' if generated_has_lf_only else 'Unknown'}")

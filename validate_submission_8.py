"""Quick validation of submission (8).csv"""

import pandas as pd

print("="*70)
print("VALIDATING: submission (8).csv")
print("="*70)

# Load files
sub = pd.read_csv('submission (8).csv')
template = pd.read_csv('sample_submission.csv')

print(f"\n✅ LOADED FILES:")
print(f"   submission (8).csv: {len(sub):,} rows")
...
if template_id_list == sub_id_list:
    ...
    for i, (t_id, s_id) in enumerate(zip(template_id_list, sub_id_list)):
        if t_id != s_id:
            ...
            print(f"      Submission (8): {s_id}")
            break

# Check coordinates
print(f"\n📍 COORDINATE CHECKS:")
print(f"   X range: [{sub['x'].min():.2f}, {sub['x'].max():.2f}] (valid: 0-120)")
print(f"   ✅ X in range: {sub['x'].between(0, 120).all()}")
print(f"   Y range: [{sub['y'].min():.2f}, {sub['y'].max():.2f}] (valid: 0-53.3)")
print(f"   ✅ Y in range: {sub['y'].between(0, 53.3).all()}")
...
# Binary format check
print(f"\n💾 BINARY FORMAT:")
with open('submission (8).csv', 'rb') as f:
    content = f.read()
...
all_good = (
    ...
    sub['x'].between(0, 120).all() and
    sub['y'].between(0, 53.3).all() and
    crlf == 0 and lf > 0
)

if all_good and template_id_list == sub_id_list:
    print("✅ PERFECT! All checks passed!")
    print("✅ submission (8).csv is ready for Kaggle!")
elif all_good:

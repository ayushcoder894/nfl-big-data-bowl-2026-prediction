# NFL Big Data Bowl 2026 - GPU-Optimized Kaggle Submission Code
...
# ===================================
# 2. GPU-OPTIMIZED NEURAL NETWORK
# ===================================
...
class AdvancedNeuralNet(nn.Module):
    def __init__(self, input_size, hidden_sizes=[512, 256, 128, 64], dropout_rate=0.3):
        ...
        layers.append(nn.Linear(prev_size, 2))  # Output x, y coordinates
...
def create_features(df):
    ...
    df['route_depth'] = np.abs(df['x'] - 30)
    df['route_width'] = np.abs(df['y'] - 26.65)
    
    # Coverage features (for defense)
    df['coverage_leverage'] = np.where(df['is_defense'] == 1, 
                                      np.sign(df['y'] - 26.65), 0)
...
# ===================================
# 5. TRAIN GPU-OPTIMIZED MODELS FOR SUB-0.1 RMSE
# ===================================
...
print("Training GPU-optimized models...")

# GPU Neural Network Training
print("Training GPU Neural Network...")
input_size = X_train.shape[1]
gpu_model = AdvancedNeuralNet(input_size, hidden_sizes=[512, 256, 128, 64], dropout_rate=0.3).to(device)
...
optimizer = optim.Adam(gpu_model.parameters(), lr=0.001, weight_decay=1e-5)
...
print(f"Training on {len(X_train)} samples with batch size {batch_size}")
...
for epoch in range(num_epochs):
    ...
    gpu_model.eval()
    with torch.no_grad():
        val_outputs = gpu_model(X_val_tensor)
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(gpu_model.state_dict(), 'best_gpu_model.pth')
    
    if epoch % 10 == 0:
        print(f'Epoch {epoch}/{num_epochs}, Train Loss: {train_loss/len(dataloader):.6f}, Val RMSE: {val_rmse:.6f}')

# Load best model
gpu_model.load_state_dict(torch.load('best_gpu_model.pth'))
gpu_model.eval()

print(f"Best GPU model validation RMSE: {torch.sqrt(best_val_loss):.6f} yards")

# Model 1: GPU-optimized XGBoost (with GPU tree method)
...
# Model 2: GPU-optimized LightGBM
print("Training GPU LightGBM...")
...
# Model 3: GPU CatBoost
print("Training GPU CatBoost...")
...
# Additional CPU models for ensemble diversity
...
# Evaluate all models
models = {
    'GPU_Neural_Network': None,  # Will handle separately
    'GPU_XGBoost': xgb_model,
    'GPU_LightGBM': lgb_model,
    'GPU_CatBoost': cb_model,
    'ExtraTrees': et_model,
...
# Evaluate GPU Neural Network
with torch.no_grad():
    gpu_pred = gpu_model(X_val_tensor).cpu().numpy()
    gpu_mse = mean_squared_error(y_val, gpu_pred)
    gpu_rmse = np.sqrt(gpu_mse)
    model_scores['GPU_Neural_Network'] = gpu_rmse
    print(f"GPU_Neural_Network RMSE: {gpu_rmse:.6f} yards")
...
# Create sophisticated GPU-enhanced ensemble
print("\nCreating GPU-enhanced weighted ensemble...")
sorted_models = sorted(model_scores.items(), key=lambda x: x[1])

# Use inverse RMSE for weights
...
for i, ((name, rmse), weight) in enumerate(zip(sorted_models, normalized_weights)):
    print(f"{i+1}. {name}: RMSE={rmse:.6f}, Weight={weight:.3f}")

def gpu_ensemble_predict(X_test):
    """GPU-enhanced ensemble prediction"""
    ...
    with torch.no_grad():
        gpu_pred = gpu_model(X_test_tensor).cpu().numpy()
        predictions.append(gpu_pred)
        weights.append(1.0 / model_scores['GPU_Neural_Network'])
    ...
    for name, model in models.items():
        if model is not None:
            try:
                ...
                weights.append(1.0 / model_scores[name])
            except:
                continue
...
ensemble_pred = gpu_ensemble_predict(X_val)
ensemble_mse = mean_squared_error(y_val, ensemble_pred)
ensemble_rmse = np.sqrt(ensemble_mse)

print(f"\nFinal GPU-Enhanced Ensemble Results:")
...
if not os.path.exists(test_input_path) or not os.path.exists(test_path):
    raise FileNotFoundError("Test files not found. Please ensure test_input.csv and test.csv are in the correct location.")
...
def generate_predictions_gpu_ensemble(test_input_df, test_targets_df, ensemble_predict_func, ensemble_info):
    """Generate predictions using GPU-enhanced ensemble model"""
    ...
    for (game_id, play_id), play_targets in test_targets_df.groupby(['game_id', 'play_id']):
        ...
        for _, target_row in play_targets.iterrows():
            ...
            if len(player_input) == 0:
                ...
                # Calculate sequence-based features (same as training)
                ...
                # Prepare feature vector (same process as training)
                feature_dict = {}
                ...
                # Predict using GPU-enhanced ensemble
...
# Generate predictions using GPU-enhanced ensemble
print("Generating predictions with GPU-enhanced ensemble model...")
submission = generate_predictions_gpu_ensemble(test_input_features, test_targets, gpu_ensemble_predict, ensemble_info)
...
print(f"\nGPU-Enhanced Submission Summary:")
...
print("\n" + "="*60)
print("🚀 GPU-OPTIMIZED SUBMISSION.CSV CREATED SUCCESSFULLY! 🚀")
print("="*60)
print("File saved as: submission_gpu.csv")
print("GPU acceleration enabled for maximum performance!")
